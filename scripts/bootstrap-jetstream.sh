...
STREAM_SUBJECTS="${NATS_STREAM_SUBJECTS:-events.auth,security.events}"
CONSUMER_NAME="${NATS_CONSUMER_NAME:-HAYABUSA_INGEST}"
...
if nats --server "${NATS_URL}" stream info "${STREAM_NAME}" >/dev/null 2>&1; then
  echo "[nats-init] Updating stream subjects: ${STREAM_NAME} (${STREAM_SUBJECTS})"
